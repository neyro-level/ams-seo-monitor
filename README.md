...
- `main` и production: действующий SEO Монитор.
- Stacked PR #107-#116: модульное ядро, строгие назначения, RLS, Инструменты, Исследования, XMLRiver worker, OAuth/MCP, кабинет и PWA.
- Эти изменения реализованы и локально проверены, но не считаются production до последовательного review, Merge Gate, merge и отдельной команды на release.
- АМС Лиды, Договоры, Счета, Презентации и Клон сайтов пока не реализованы.
- Переезд production на Timeweb Managed PostgreSQL 18 не выполнялся.
