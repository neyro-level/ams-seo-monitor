...
- application: idempotent enqueue, payload validation, retry policy and queue-aware job lifecycle;
- infrastructure: Prisma reliability repository, pg-boss runtime client, retention runner and bounded topic dispatcher;
- worker entrypoint экспортируется отдельно от framework-neutral API и использует JobPrincipal for async handlers.
