...
function SortLink({ field, label, query }: { field: ProjectTableQuery["sort"]; label: string; query: ProjectTableQuery }) {
  ...
  return <Link className="inline-flex min-h-11 items-center gap-1 text-left text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]" href={projectHref(query, { page: 1, sort: field, direction })}>{label}{active ? query.direction === "asc" ? <ChevronUp className="size-3.5" aria-hidden /> : <ChevronDown className="size-3.5" aria-hidden /> : <ChevronDown className="size-3.5 opacity-30" aria-hidden />}</Link>;
}
...
export function ProjectTable({ result, options, query }: { result: ProjectListResult; options: ProjectFormOptions; query: ProjectTableQuery }) {
  const columns = useMemo<ColumnDef<typeof impulseTableFeatures, ProjectListItem, unknown>[]>(() => [
    { accessorKey: "name", header: () => <SortLink field="name" label="Проект" query={query} />, cell: ({ row }) => <div><p className="font-semibold text-[var(--foreground)]">{row.original.name}</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">{row.original.slug}</p></div> },
    { accessorKey: "organizationName", header: "Организация", cell: ({ row }) => <span className="text-sm text-[var(--text-secondary)]">{row.original.organizationName}</span> },
    ...
    { accessorKey: "updatedAt", header: () => <SortLink field="updatedAt" label="Обновлён" query={query} />, cell: ({ row }) => <time className="block whitespace-nowrap text-right text-sm tabular-nums text-[var(--text-secondary)]" dateTime={row.original.updatedAt}>{new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium" }).format(new Date(row.original.updatedAt))}</time> },
  ...
  return <AdminDataTable ariaLabel="Список проектов" columns={columns} data={result.items} filtersActive={Boolean(query.search || query.status)} emptyTitle="Проектов пока нет" emptyDescription="Создайте первый проект в форме ниже." filteredEmptyDescription="Измените фильтры или сбросьте их." page={result.page} pageCount={pageCount} previousHref={projectHref(query, { page: Math.max(1, result.page - 1) })} nextHref={projectHref(query, { page: Math.min(pageCount, result.page + 1) })} minWidth="980px" mobileRenderer={(project) => <article className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--card)] p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold text-[var(--foreground)]">{project.name}</h2><p className="mt-1 text-xs text-[var(--muted-foreground)]">{project.slug}</p></div><ProjectStatusBadge status={project.status} /></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-[var(--muted-foreground)]">Организация</dt><dd className="mt-1 text-[var(--foreground)]">{project.organizationName}</dd></div><div><dt className="text-xs text-[var(--muted-foreground)]">Версия</dt><dd className="mt-1 tabular-nums text-[var(--foreground)]">{project.version}</dd></div></dl><div className="mt-4 border-t border-[var(--border)] pt-4"><ProjectRowActions key={`${project.id}:${project.version}`} project={project} options={options} /></div></article>} />;
}
