...
export function ProjectTable({ result, options, query }: { result: ProjectListResult; options: ProjectFormOptions; query: ProjectTableQuery }) {
  const columns = useMemo<ColumnDef<typeof impulseTableFeatures, ProjectListItem, unknown>[]>(() => [
    { accessorKey: "name", header: () => <SortLink field="name" label="Проект" query={query} />, cell: ({ row }) => <div><p className="font-semibold text-app-foreground">{row.original.name}</p><p className="mt-1 text-xs text-app-muted-foreground">Код: {row.original.slug}</p></div> },
