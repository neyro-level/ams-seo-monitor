...
function MembershipEditCard({ item }: { item: MembershipListItem }) {
  ...
  return <SectionCard title={item.userName} description={`${item.userEmail} · ${item.organizationName}`}><div className="space-y-4"><form className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]" onSubmit={submitUpdate}><input type="hidden" {...updateForm.register("organizationId")} /><input type="hidden" {...updateForm.register("membershipId")} /><input type="hidden" {...updateForm.register("version", { valueAsNumber: true })} /><FormField error={updateForm.formState.errors.tenantRole?.message} label="Роль в организации" required><SelectInput options={tenantRoleOptions} {...updateForm.register("tenantRole")} /></FormField><div className="self-end"><SubmitRow busy={updateForm.formState.isSubmitting} feedback={updateFeedback} label="Сохранить" onRefresh={() => router.refresh()} pendingLabel="Сохраняем…" variant="outline" /></div></form><form className="flex flex-wrap items-center gap-3 border-t border-[var(--border)] pt-4" onSubmit={submitRemove}><input type="hidden" {...removeForm.register("organizationId")} /><input type="hidden" {...removeForm.register("membershipId")} /><input type="hidden" {...removeForm.register("version", { valueAsNumber: true })} /><SubmitRow busy={removeForm.formState.isSubmitting} feedback={removeFeedback} label={`Отозвать доступ: ${item.organizationName}`} onRefresh={() => router.refresh()} pendingLabel="Отзываем…" variant="ghost" /></form></div></SectionCard>;
}
