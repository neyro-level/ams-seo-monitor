...
const tenantRoleLabels = { ORG_OWNER: "Владелец", ORG_MEMBER: "Участник", VIEWER: "Наблюдатель" } as const;
...
function SiteFields({ form, index, canRemove, onRemove }: { form: UseFormReturn<ProvisionClientInput>; index: number; canRemove: boolean; onRemove: () => void }) {
  ...
  return <SectionCard title={`Сайт ${index + 1}`} description="Для каждого сайта задаются поисковые запросы и регион проверки в Яндексе и Google."><FieldGroup><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><FormField label="Название сайта" required error={errors?.name?.message}><TextInput {...form.register(`sites.${index}.name`)} /></FormField><FormField helper="Короткое уникальное имя латиницей для адреса отчёта." label="Код сайта" required error={errors?.slug?.message}><TextInput {...form.register(`sites.${index}.slug`)} /></FormField><FormField label="Адрес сайта" required error={errors?.url?.message}><TextInput inputMode="url" {...form.register(`sites.${index}.url`)} /></FormField><FormField label="Часовой пояс" required error={errors?.timezone?.message}><TextInput placeholder="Europe/Moscow" {...form.register(`sites.${index}.timezone`)} /></FormField><div className="sm:col-span-2"><FormField label="Регион проверки позиций" required error={errors?.regionName?.message} helper={regionName ? `Выбран: ${regionName}` : "Регион проверяется отдельно для Яндекса и Google."}><RegionPicker onSelect={selectRegion} /></FormField></div><div className="sm:col-span-2 xl:col-span-3"><FormField label="Поисковые запросы" required error={errors?.queries?.message} helper={`${queries.length}/100 запросов, минимум 20. По одному запросу в строке.`}><AreaInput rows={8} value={queries.join("\n")} onChange={(event) => form.setValue(`sites.${index}.queries`, parseQueries(event.target.value), { shouldDirty: true, shouldValidate: true })} /><label className="mt-2 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-[var(--radius)] border border-[var(--border)] px-3 text-sm font-medium"><Upload className="size-4" aria-hidden />Загрузить файл .txt или .csv<input className="sr-only" type="file" accept=".txt,.csv,text/plain,text/csv" onChange={(event) => void loadQueries(event.target.files?.[0])} /></label></FormField></div></div>{canRemove ? <Button className="self-start" type="button" variant="outline" onClick={onRemove}><Trash2 data-icon="inline-start" aria-hidden />Удалить сайт</Button> : null}</FieldGroup></SectionCard>;
}
