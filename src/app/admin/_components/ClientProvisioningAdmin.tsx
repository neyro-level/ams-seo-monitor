...
function SiteFields({ form, index, canRemove, onRemove }: { form: UseFormReturn<ProvisionClientInput>; index: number; canRemove: boolean; onRemove: () => void }) {
  ...
  return <SectionCard title={`Сайт ${index + 1}`} description="Для каждого сайта задаётся отдельное ядро и четыре цели проверки: Яндекс/Google × desktop/mobile."><FieldGroup><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><FormField label="Название сайта" required error={errors?.name?.message}><TextInput {...form.register(`sites.${index}.name`)} /></FormField><FormField label="Slug сайта" required error={errors?.slug?.message}><TextInput {...form.register(`sites.${index}.slug`)} /></FormField><FormField label="HTTPS-адрес" required error={errors?.url?.message}><TextInput inputMode="url" {...form.register(`sites.${index}.url`)} /></FormField><FormField label="Timezone" required error={errors?.timezone?.message}><TextInput placeholder="Europe/Moscow" {...form.register(`sites.${index}.timezone`)} /></FormField><div className="sm:col-span-2"><FormField label="Регион Topvisor" required error={errors?.regionName?.message} helper={regionName ? `Выбран: ${regionName}` : "Поиск сверяет регион отдельно для Яндекса и Google."}><RegionPicker onSelect={selectRegion} /></FormField></div><div className="sm:col-span-2 xl:col-span-3"><FormField label="SEO-ядро" required error={errors?.queries?.message} helper={`${queries.length}/100 запросов, минимум 20. По одному запросу в строке.`}><AreaInput rows={8} value={queries.join("\n")} onChange={(event) => form.setValue(`sites.${index}.queries`, parseQueries(event.target.value), { shouldDirty: true, shouldValidate: true })} /><label className="mt-2 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-[var(--radius)] border border-[var(--border)] px-3 text-sm font-medium"><Upload className="size-4" aria-hidden />Загрузить UTF-8 .txt/.csv<input className="sr-only" type="file" accept=".txt,.csv,text/plain,text/csv" onChange={(event) => void loadQueries(event.target.files?.[0])} /></label></FormField></div></div>{canRemove ? <Button className="self-start" type="button" variant="outline" onClick={onRemove}><Trash2 data-icon="inline-start" aria-hidden />Удалить сайт</Button> : null}</FieldGroup></SectionCard>;
}
...
export function ClientProvisioningAdmin({ users, thresholdProfiles, clusterProfiles }: { users: IdentityAdminUserListItem[]; thresholdProfiles: Array<{ id: string; label: string }>; clusterProfiles: Array<{ id: string; label: string }> }) {
  ...
  return <div className="flex flex-col gap-6"><SectionCard title="Создать клиента" description="Организация, проект, сайты, пользователь, доступ и задания интеграций создаются одной атомарной операцией."><form className="flex flex-col gap-6" onSubmit={submit}><FieldGroup><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><FormField error={form.formState.errors.organizationName?.message} label="Организация" required><TextInput {...form.register("organizationName")} /></FormField><FormField error={form.formState.errors.organizationSlug?.message} label="Slug организации" required><TextInput {...form.register("organizationSlug")} /></FormField><FormField error={form.formState.errors.projectName?.message} label="Проект" required><TextInput {...form.register("projectName")} /></FormField><FormField error={form.formState.errors.projectSlug?.message} label="Slug проекта" required><TextInput {...form.register("projectSlug")} /></FormField><FormField error={form.formState.errors.thresholdProfileId?.message} label="Пороговый профиль" required><SelectInput options={thresholdProfiles.map((item) => ({ value: item.id, label: item.label }))} {...form.register("thresholdProfileId")} /></FormField><FormField error={form.formState.errors.clusterProfileId?.message} label="Кластерный профиль" required><SelectInput options={clusterProfiles.map((item) => ({ value: item.id, label: item.label }))} {...form.register("clusterProfileId")} /></FormField><FormField error={form.formState.errors.userName?.message} label="Имя пользователя" required><TextInput {...form.register("userName")} /></FormField><FormField error={form.formState.errors.username?.message} label="Логин" required><TextInput autoCapitalize="none" autoComplete="off" {...form.register("username")} /></FormField><FormField error={form.formState.errors.password?.message} helper="Ровно 8 печатных символов без пробелов." label="Пароль" required><TextInput autoComplete="new-password" maxLength={8} minLength={8} type="password" {...form.register("password")} /></FormField><FormField error={form.formState.errors.tenantRole?.message} label="Роль" required><SelectInput options={tenantRoleOptions} {...form.register("tenantRole")} /></FormField></div></FieldGroup>{sites.fields.map((site, index) => <SiteFields form={form} index={index} key={site.id} canRemove={sites.fields.length > 1} onRemove={() => sites.remove(index)} />)}<Button className="self-start" type="button" variant="outline" disabled={sites.fields.length >= 50} onClick={() => sites.append({ ...emptySite })}><Plus data-icon="inline-start" aria-hidden />Добавить сайт</Button><SubmitRow busy={form.formState.isSubmitting} feedback={feedback} label="Создать клиента и настроить сайты" onRefresh={() => router.refresh()} pendingLabel="Создаём…" /></form></SectionCard><div className="grid gap-4 xl:grid-cols-2">{users.map((user) => <UserAccessCard key={user.id} user={user} />)}</div></div>;
}
