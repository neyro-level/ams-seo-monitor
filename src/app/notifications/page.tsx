...
const categoryLabels = {
  ...
  QUEUE: "Обновление данных",
  ACCESS: "Доступ",
} satisfies Record<(typeof notificationCategorySchema.options)[number], string>;
...
export default async function NotificationsPage({ searchParams }: { searchParams: Search }) {
  ...
  return <div className="flex flex-col gap-6"><PageHeader title="Уведомления" description="Подключения, обновления отчётов и события, которые требуют внимания." /><form className="grid gap-3 rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--card)] p-4 sm:grid-cols-2 xl:grid-cols-5"><NativeSelect aria-label="Состояние" name="state" defaultValue={query.state}><NativeSelectOption value="all">Все</NativeSelectOption><NativeSelectOption value="unread">Непрочитанные</NativeSelectOption><NativeSelectOption value="attention">Требуют внимания</NativeSelectOption></NativeSelect><NativeSelect aria-label="Организация" name="organizationId" defaultValue={query.organizationId ?? ""}><NativeSelectOption value="">Все организации</NativeSelectOption>{options.organizations.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.name}</NativeSelectOption>)}</NativeSelect><NativeSelect aria-label="Проект" name="projectId" defaultValue={query.projectId ?? ""}><NativeSelectOption value="">Все проекты</NativeSelectOption>{options.projects.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.name}</NativeSelectOption>)}</NativeSelect><NativeSelect aria-label="Сайт" name="siteId" defaultValue={query.siteId ?? ""}><NativeSelectOption value="">Все сайты</NativeSelectOption>{options.sites.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.name}</NativeSelectOption>)}</NativeSelect><NativeSelect aria-label="Категория" name="category" defaultValue={query.category ?? ""}><NativeSelectOption value="">Все категории</NativeSelectOption>{notificationCategorySchema.options.map((item) => <NativeSelectOption key={item} value={item}>{categoryLabels[item]}</NativeSelectOption>)}</NativeSelect><Button type="submit">Применить фильтры</Button></form><NotificationFeed items={result.items} /><Pagination page={result.page} pageCount={pageCount} previousHref={href(current, Math.max(1, result.page - 1))} nextHref={href(current, Math.min(pageCount, result.page + 1))} /></div>;
}
