...
export function DirectorReportTabs({ snapshot, analytics, timezone }: { snapshot: SiteReportSnapshot; analytics: DirectorAnalytics | null; timezone: string }) {
  ...
  const selectors = <div className="flex flex-wrap gap-2" aria-label="Поисковая система и устройство">
    ...
    {(["ALL", "DESKTOP", "MOBILE"] as const).map((value) => <Button key={value} type="button" size="sm" variant={device === value ? "secondary" : "outline"} aria-pressed={device === value} onClick={() => setDevice(value)}>{value === "ALL" ? "Все устройства" : value === "DESKTOP" ? "Desktop" : "Mobile"}</Button>)}
  </div>;
  return <Tabs defaultValue="overview">
    ...
    <TabsContent value="overview" className="space-y-4">
      ...
      <SectionCard title="Воронка SEO" note={`Сводка разных источников · ${timezone}`}><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Показы Вебмастера", webmaster?.summary.shows], ["Клики Вебмастера", webmaster?.summary.clicks], ["Визиты Метрики", metrica?.summary.visits], ["Уникальные целевые визиты", metrica?.summary.targetVisits]].map(([label, value]) => <div className="rounded-[var(--radius)] bg-[var(--muted)] p-4" key={String(label)}><p className="text-xs text-[var(--muted-foreground)]">{label}</p><p className="mt-2 text-2xl font-semibold tabular-nums">{formatInteger(typeof value === "number" ? value : null)}</p></div>)}</div><p className="mt-3 text-xs text-[var(--muted-foreground)]">Этапы рассчитаны разными системами и не являются прямой атрибуцией одного визита.</p></SectionCard>
    </TabsContent>
    <TabsContent value="queries" className="space-y-4">
      ...
      {webmasterQueries.length ? <SectionCard title="Спрос и видимость в Яндексе" note="Вебмастер · средняя позиция не заменяет Topvisor"><DataTable caption="Запросы Вебмастера" columns={["Запрос", "Устройство", "Спрос", "Показы", "Клики", "CTR", "Позиция", "Страница"]} rows={webmasterQueries.slice(0, 30).map((row, index) => ({ key: `${row.query}-${row.device}-${index}`, cells: [row.query, row.device, formatInteger(row.demand), formatInteger(row.shows), formatInteger(row.clicks), formatPercent(row.ctr), formatPosition(row.averagePosition), row.relevantUrl ?? "—"] }))} /></SectionCard> : engine === "YANDEX" ? <StatePanel state="empty" title="Запросы ещё не собраны" description="Данные появятся после очередного обновления Вебмастера." /> : null}
    ...
    <TabsContent value="competitors" className="space-y-4">
      {selectors}
      {competitors.length ? <SectionCard title="Ведущие домены" note={`Topvisor · снимок ${analytics?.competitorCapturedAt ? new Date(analytics.competitorCapturedAt).toLocaleString("ru-RU", { timeZone: timezone }) : "—"}`}><DataTable caption="Конкуренты по фактической выдаче ядра" columns={["Система", "Устройство", "Домен", "Видимость", "Средняя позиция", "Топ‑3", "Топ‑10", "Топ‑30"]} rows={competitors.map((row, index) => ({ key: `${row.engine}-${row.device}-${row.domain}-${index}`, cells: [row.engine === "YANDEX" ? "Яндекс" : "Google", row.device === "DESKTOP" ? "Desktop" : "Mobile", row.domain, formatPercent(row.visibility), formatPosition(row.averagePosition), formatInteger(row.top3), formatInteger(row.top10), formatInteger(row.top30)] }))} /></SectionCard> : <StatePanel state="empty" title="Снимок конкурентов ещё не готов" description="Первый снимок появится после настройки Topvisor, затем обновляется в первый понедельник месяца." />}
    </TabsContent>
  </Tabs>;
}
